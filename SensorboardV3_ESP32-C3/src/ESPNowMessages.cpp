...
ReedBoardMessage reedMsg;
SensorboardV2Message sensorboardV2Msg;
...
void ESPNowPrintDebug(ESPNowBuffer *buffer, Print *print)
{
    ...
    if (type == BoardMessageType::REED_BOARD_MESSAGE)
    {
        reedMsg.parseData(buffer);
        reedMsg.printDebug(print);
    }
    else if (type == BoardMessageType::SENSORBOARD_V2_MESSAGE)
    {
        sensorboardV2Msg.parseData(buffer);
        sensorboardV2Msg.printDebug(print);
    }
    else
    {
